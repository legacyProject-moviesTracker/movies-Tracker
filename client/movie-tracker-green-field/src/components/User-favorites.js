    ...
    async function fetchMovies() {
      try {
        const response = await axios.get(
          `http://localhost:8080/user/favorites/${userId}`,
          {
            headers: {
        ...
        setMovies(response.data); // Set the movies state with the response
      } catch (err) {
  ...
  const handleDelete = async (movieId) => {
    try {
      const response = await axios.delete(
        `http://localhost:8080/user/favorite`,
        {
          data: { userId, favoriteMovieId: movieId }, // Pass data in request body
          headers: {
            Authorization: `Bearer ${token}`,
          },
        }
